...
app.get("/random", async(req,res)=>{
  try{
    const random = await Math.floor(Math.random()*jokes.length);
...
app.get("/jokes/:id", async(req, res)=>{
  try{
    const requestedId = await parseInt(req.params.id);
    const foundjoke = jokes.find((joke) => joke.id ===requestedId);
    res.json(foundjoke);
  }catch(error){
    console.log("Error Fetching Data");
  }
...
app.get("/filter", async(req,res)=>{
  try{
    ...
    const filterJokes = jokes.filter((joke)=> joke.jokeType===filter);
